...
2. **Python-скрипт (`oauth/omyandex.py`), реализующий две ключевые команды**:
   ...
   Важно! Имя файла должно начинаться с префикса `om`, а оставшаяся часть имени должна соответствовать внутреннему названию метода авторизации из XML. Расширение `.py` отбрасывается при установке.
...
- [Страница регистрации приложения](https://oauth.yandex.ru/client/new/id)
- Платформы приложений: **Веб-сервис**
- Redirect URI:
  ```
...
В результате ваше приложение должно иметь подобные настройки
![alt text](assets/yandex_app.png)

CLIENT_ID и CLIENT_SECRET приложения нужно прописать в файле `oauth/omyandex.py`.
...
3. Перейдите в каталог проекта, cоберите и установите модуль командой:
   ```
   make install
   ```